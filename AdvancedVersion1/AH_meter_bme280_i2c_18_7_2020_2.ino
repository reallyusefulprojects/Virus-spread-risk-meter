...
// MESUREMENT ACQUISITION
  ...
  // Air pressure hpa
  float pressavg = (bme.readPressure() / 100.0F - 0.60);
...
repeat(3) {
  ...
  lcd.clear();
     ...
     if (mr < 6) lcd.print("Waarschuwing !!!");
                 lcd.setCursor(0, 1);
                 lcd.print("Gevaar!!");

     if (mr < 7.5 > 6 ) lcd.print("Let Op !!!");
                        lcd.setCursor(0, 1);
                        lcd.print("Gevaar Dreigd");
                
     if (mr > 7.5) lcd.print("Virus versprei-");
                   lcd.setCursor(0, 1);
